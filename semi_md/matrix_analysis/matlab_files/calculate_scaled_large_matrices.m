calculate_scaled_large_matrices_("randomSparse200eigs.mat", [-2, -1, 0]);

function calculate_scaled_large_matrices_(filename, scaling)
tic;
load(filename);
t_load = toc
compute_and_display(-D, V, scaling);

end

function [fcA, fsA, A] = wkm_vpa (D, V, scale)
  fc =@(x) cosh(sqrt(x));
  sinhc =@(x) sinh(x)./x;
  fs =@(x) sinhc(sqrt(x));
  ...
  fcA = double(V*diag(fc(diag(D) * 10^scale))/V);
  fsA = double(V*diag(fs(diag(D) * 10^scale))/V);
  A = double(V*(-D * 10^scale)/V);
  digits(d_old);
end
...
function compute_and_display (D, V, scaling)
for scale = scaling
  % Compute the fc(A) and fs(A) in vpa to compute approximation errors.
  tic;
  [fcA, fsA, A] = wkm_vpa(D, V, scale);
  t_vpa = toc
  ...
  filename = sprintf('randomSparse%d_%f.mat', length(V), scale);
  save(filename, 'A', 'fcA', 'fsA', 'condc', 'conds', 'err_cA', 'err_sA');
