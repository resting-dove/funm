...
def prepare_starting_vector(evecs, n:int):
    ...
    evecs2 = evecs.reshape((evecs.shape[0], n, n, n))
    ret = (evecs2 / factor.reshape((evecs.shape[0], n, n, n))).sum(axis=-1).sum(axis=-1).sum(axis=-1)
...
def get_lanczos_errors(V, H, beta, exact, matfunc, step=1, upper=100):
    ...
    for i in idx[1:]:
        ...
        errors[i] = np.linalg.norm(f.flatten()-exact)
    return errors
...
if __name__ == "__main__":
    ...
    u0 = prepare_starting_vector(evecs, n)
    exact = (evecs @ np.diagflat(np.exp(evals)) @ evecs.T @ u0.flatten())
    fig, ax = plt.subplots()
    for krylov_size in [20, 10, 6]:
        num_starts = 50 // krylov_size + 1

        # Calculate the matrix exponential
        npfs, npupdate_norms, final_size = lanczos_method(t * A.toarray(), u0.flatten(), scipy.sparse.linalg.expm,
                                                          krylov_size=krylov_size, max_starts=num_starts,
                                                          stopping_acc=-np.inf, arnoldi_acc=-np.inf, stopping_decay=-np.inf)
    ...
    (w, V, H, m) = arnoldi(t * A.toarray(), u0.flatten() / beta, N, trunc=1)
    idx, lanczos_errors = get_lanczos_errors(V, H, beta, exact, scipy.linalg.expm, 2, upper=300)
    plot_store["Lanczos idx"] = idx
    plot_store["Lanczos error_norms"] = lanczos_errors
    np.savez(open(os.path.join(root_path, "artifacts", "plot_store"), "wb"), **plot_store)

    lanc_plot = ax.plot(idx, lanczos_errors, color='black')

    ax.set_title(f"Error for heat equation with n={n}")
    ax.set_yscale("log")
    ax.set_ylim(bottom=max(np.finfo(evals[0].dtype).eps, plt.ylim()[0]) / 100)
    ax.set_xlabel("Arnoldi iterations")
    fig.legend(framealpha=.5, scatterpoints=1, numpoints=1)
