...
class cos_sqrt_matlab():
    def __init__(self):
        self.eng = matlab.engine.start_matlab()
        self.eng.addpath("/home/lara/funm/matlab_integration")
        self.eng.addpath("/home/lara/funm/matlab_integration/wkm")

    def __call__(self, H: scipy.sparse.csr_array):
        C = self.eng.tridiag_cos(H.diagonal(-1), H.diagonal(), H.diagonal(1))
        return np.asarray(C)


def cos_sqrtm(H: np.ndarray):
    # Assume H is symmetric tridiagonal
    return scipy.linalg.cosm(scipy.linalg.sqrtm(H))
...
if __name__ == "__main__":
    ...
    colors = Colors()
    markers = ["o", "^"]
    for j, krylov_size in enumerate([10, 20]):
    ...
    np.savez(
        os.path.join(root_path, "artifacts", "plot_store" + f"_bounds_cos_sqrt_uniform_{n}_restarts_{norm_name}-norm"),
        **plot_store)
    ...
    fig.savefig(os.path.join(root_path, f"figures/bounds_cos_sqrt_uniform_{n}_restarts_{norm_name}-norm.png"))
    plt.show()
    1 + 1
