...
if __name__ == "__main__":
    ...
    for krylov_size in [20, 10, 6]:
        ...
        npfs, npupdate_norms, final_size = lanczos_method(t * A, u0.flatten(), diag_expm,
    ...
    idx, lanczos_errors = get_lanczos_errors(V, scipy.sparse.csc_array(H), beta, exact, diag_expm, 2,
                                             upper=300)
    ...
    np.savez(os.path.join(root_path, "artifacts", "plot_store" + f"heat{n}_diag"), **plot_store)
    ...
    fig.savefig(os.path.join(root_path, f"figures/heat{n}_diag_ErrorPlot.png"))
    plt.show()
    1 + 1
