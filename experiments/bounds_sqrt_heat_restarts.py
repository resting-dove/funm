...
def sqrtm_sym(T: scipy.sparse.csr_array):
    w, v = scipy.linalg.eigh_tridiagonal(T.diagonal(), T.diagonal(-1))
...
if __name__ == "__main__":
    ...
    func_dense = scipy.linalg.sqrtm
    func_sparse = sqrtm
    func_sparse_sym = sqrtm_sym
    func_scalar = np.sqrt
    ...
    colors = Colors()
    markers = ["o", "^"]
    for j, krylov_size in enumerate([40, 160]):
    ...
    np.savez(os.path.join(root_path, "artifacts", "plot_store" + f"_bounds_sqrt_heat_{n}_restarts_{norm_name}-norm"),
             **plot_store)
    ...
    fig.savefig(os.path.join(root_path, f"figures/bounds_sqrt_heat_{n}_restarts_{norm_name}-norm.png"))
    plt.show()
    1 + 1
