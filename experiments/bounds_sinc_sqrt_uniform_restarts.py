...
class sinc_sqrt_matlab():
    ...
    def __call__(self, H: scipy.sparse.csr_array):
        S = self.eng.tridiag_sinc(H.diagonal(-1), H.diagonal(), H.diagonal(1), nargout=1)
        return np.asarray(S)


def sinc_sqrtm(H: np.ndarray):
    w, v = scipy.linalg.eig(H)
    sqrt_w = np.real(np.sqrt(w))
    S = v @ np.diagflat(np.sinc(sqrt_w / np.pi)) @ v.T
    return S
...
if __name__ == "__main__":
    ...
    np.savez(
        os.path.join(root_path, "artifacts", "plot_store" + f"_bounds_sinc_sqrt_uniform_{n}_restarts_{norm_name}-norm"),
        **plot_store)
    ...
    fig.savefig(os.path.join(root_path, f"figures/bounds_sinc_sqrt_uniform_{n}_restarts_{norm_name}-norm.png"))
    plt.show()
    1 + 1
